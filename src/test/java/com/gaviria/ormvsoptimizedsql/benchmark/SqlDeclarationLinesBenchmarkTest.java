...
class SqlDeclarationLinesBenchmarkTest {
    ...
    private long runOnce(Long accId, int year, int month, int page, int size) {
        ...
        Page<DeclarationLineDTO> out = service.declarationLines(accId, year, month, page, size);
        ...
        System.out.printf(
                "SQL C p=%d s=%d -> Time: %.3fs | Queries total=%d select=%d | items=%d%n",
                page, size, (t1 - t0) / 1e9, qc.getTotal(), qc.getSelect(), out.getContent().size()
        );
        return t1 - t0;
    }

    @Test
    void benchmarkSqlDeclarationLines() {
        Long accId = 1L;
        ...
        System.out.printf("SQL C median over 3 runs: %.3fs%n", medianNs / 1e9);
